...
### Step 1: Ten Stress Ratios

Ten financial ratios covering six risk dimensions are calculated for each bank
from FDIC call report data:

| Ratio | Risk Dimension | Stress Direction |
|-------|---------------|-----------------|
| Leverage Ratio | Solvency | Low = stressed |
| Texas Ratio | Asset Quality | High = stressed |
| Securities Ratio | Rate Risk | High = stressed |
| Uninsured Deposits | Run Risk | High = stressed |
| Return on Assets (ROA) | Profitability | Low = stressed |
| Loan-to-Deposit | Liquidity | High = stressed |
| CRE Concentration | Concentration Risk | High = stressed |
| Net Interest Margin | Earnings Quality | Low = stressed |
| Cash Ratio | Liquidity Buffer | Low = stressed |
| Borrowings Ratio | Funding Stability | High = stressed |

### Step 2: Percentile Scoring Within Tiers

Banks are divided into four size tiers:
...
A logistic regression model is trained on 587,294 bank-quarter observations
(2003-2023) with DEFAULT (failed within 2 years) as the target variable:

- **Algorithm:** Logistic Regression (class_weight='balanced', C=0.1)
- **Features:** 20 (10 ratios + 10 QoQ trends)
- **Trend handling:** Option A — trends improve regression accuracy only,
  not included in composite score
- **Weight constraints:** 5% floor (no ratio ignored), 20% cap (ROA)
- **Training AUC:** 0.956

### Step 4: Two Composite Scores
```
Historical Score    = Σ (ratio_percentile × regression_weight)
Current Regime Score = Σ (ratio_percentile × 10%)
```

Both scores normalized to 0-100 scale within size tier.

### Step 5: Stress Classification
